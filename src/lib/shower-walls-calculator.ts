...
export class ShowerWallsCalculator {
   ...
   *
   * Design Labor Order:
   * 1. Wall Tile (Tiling Walls)
   * 2. Grouting
   * 3. Niche Installation
   * 4. Shower Door Installation
   * 5. Install Grab Bar(s)
   *
   * Construction Labor Order:
   * 1. Boarding
   * 2. Waterproofing
   * 3. Silicone
   * 4. Install Blocking for Shower Door
   * 5. Wall Repair
   * 6. Re-insulate exterior walls
   */
  static generateAutoLabor(
    ...
    const designLaborItems: LaborItem[] = [];
    const constructionLaborItems: LaborItem[] = [];
    const rate = hourlyRate ?? this.DEFAULT_HOURLY_RATE;

    // ==========================================
    // DESIGN LABOR (in order)
    // ==========================================
    ...
    // 3. Niche Installation (Design Labor)
    ...
    // 4. Shower Door Installation (Design Labor)
    if (design.showerDoor !== 'None') {
      let doorSlug = '';
      let doorInstallHours = 0;
      let doorTypeName = '';

      if (design.showerDoor === 'Sliding Glass Door') {
        doorSlug = 'sliding';
        doorInstallHours = 4.0;
        doorTypeName = 'Sliding';
      } else if (design.showerDoor === 'Pivot Glass Door') {
        doorSlug = 'pivot';
        doorInstallHours = 5.0;
        doorTypeName = 'Pivot';
      } else if (design.showerDoor === 'Custom Glass') {
        doorSlug = 'custom';
        doorInstallHours = 6.0;
        doorTypeName = 'Custom';
      }

      if (doorInstallHours > 0 && doorSlug) {
        designLaborItems.push({
          id: `sw-door-install-${doorSlug}`,
          name: `Install ${doorTypeName} Glass Door`,
          hours: doorInstallHours,
          rate,
          scope: 'showerWalls_design',
          source: 'calculated',
          isAutoGenerated: true,
    ...
    // 5. Install Grab Bar(s) (Design Labor)
    ...
    if (design.showerDoor !== 'None') {
      ...
      if (doorSlug) {
        constructionLaborItems.push({
          ...
          name: `Install Blocking for ${doorTypeName} Door`,
          hours: 2.0,
          rate,
          scope: 'showerWalls_construction',
          source: 'calculated',
    ...
    // Return design items first, then construction items
    return [...designLaborItems, ...constructionLaborItems];
  }
   ...
   * 1. Wall Tile (with size)
   * 2. Grout
   * 3. Tile Edge
   * 4. Shower Door
   *
   * Construction Materials Order:
   * 1. Thinset Mortar
   * 2. Blocking for Shower Glass Door
   * 3. Niche
   * 4. Grab Bar Blocking
   * 5. Waterproofing materials (based on selection)
  ...
  static generateAutoMaterials(
    ...
    // ==========================================
    // CONSTRUCTION MATERIALS (in order)
    // ==========================================
    ...
    if (design.waterproofingSystem !== 'None / Select System...') {
      switch (design.waterproofingSystem) {
        ...
        case 'RedGard® Waterproofing Membrane':
          materialItems.push({
            id: 'mat-wp-liquid',
            name: 'RedGard® Waterproofing Membrane',
  ...
  static calculate(
    ...
    const laborItems = this.generateAutoLabor(walls, design, hourlyRate);
