...
import { PerformedByToggle } from './PerformedByToggle';
import { ConfigurableTask } from './ConfigurableTask';
import { InfoMessage } from './InfoMessage';
import { DesignChoices, FinishingsScope, AccentWall } from '@/types/finishings';

interface FinishingsSectionProps {
  designChoices: DesignChoices;
  setDesignChoices: (
    choices: DesignChoices | ((prev: DesignChoices) => DesignChoices)
  ) => void;
  finishingsScope: FinishingsScope;
  setFinishingsScope: (
    scope: FinishingsScope | ((prev: FinishingsScope) => FinishingsScope)
  ) => void;
  accentWalls: AccentWall[];
  setAccentWalls: (
    walls: AccentWall[] | ((prev: AccentWall[]) => AccentWall[])
  ) => void;
}
...
export function FinishingsSection({
  ...
  const handleDimensionChange = (
    field: keyof DesignChoices['dimensions'],
    value: string
  ) => {
  ...
  const handleScopeChange = (
    ...
    setFinishingsScope((prev) => ({
      ...prev,
      [key]: {
        ...(prev[key] as Record<string, string | boolean | number>),
        [field]: value,
      },
    }));
  };
  ...
  const handleDeleteAccentWall = (id: string) => {
    setAccentWalls((prev) => prev.filter((wall) => wall.id !== id));
  };

  const handleAccentWallChange = (id: string, field: string, value: string) => {
    setAccentWalls((prev) =>
      prev.map((wall) => {
        if (wall.id === id) {
          if (field === 'width' || field === 'height') {
            return {
              ...wall,
              dimensions: { ...wall.dimensions, [field]: value },
            };
          }
            ...
            <div className='pl-4 space-y-3'>
              ...
              >
                <QuantityInput
                  ...
                  onChange={(v) =>
                    handleScopeChange('installSinkFaucet', 'quantity', v.toString())
                  }
                />
              </ConfigurableTask>
              <OptionToggle
