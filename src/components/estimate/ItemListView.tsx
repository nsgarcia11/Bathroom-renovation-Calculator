...
interface ItemListViewProps {
  title: string;
  itemsByCategory: { [category: string]: StructuralItem[] };
  onUpdateItem: (id: string, updatedValues: Partial<StructuralItem>) => void;
...
export function ItemListView({
  ...
  const total = Object.values(itemsByCategory)
    ...
    .reduce(
      (sum, item) =>
        sum +
        (parseFloat(item.quantity || item.hours || '0') || 0) *
          (parseFloat(item.price || item.rate || '0') || 0),
      0
    );
                      ...
                      <div className='grid grid-cols-3 gap-4 mt-2'>
                        <div>
                          ...
                          <Input
                            type='number'
                            value={
                              itemType === 'labor' ? item.hours : item.quantity
                            }
                            onChange={(e) =>
                              onUpdateItem(
                                item.id,
                        ...
                        <div>
                          ...
                          <Input
                            type='number'
                            value={
                              itemType === 'labor' ? item.rate : item.price
                            }
                            onChange={(e) =>
                              onUpdateItem(
                                item.id,
                        ...
                        <div>
                          ...
                          <p className='w-full p-2 text-center font-semibold text-slate-800'>
                            $
                            {(
                              (parseFloat(item.quantity || item.hours || '0') ||
                                0) *
                              (parseFloat(item.price || item.rate || '0') || 0)
                            ).toFixed(2)}
                          </p>
