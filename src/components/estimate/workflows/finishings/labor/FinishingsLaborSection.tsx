...
export default function FinishingsLaborSection() {
  ...
  const generateLaborItems = useCallback(() => {
    ...
    // -------------------------------------------------------------------------
    // Trim setup time (20 min / 0.33 hr) is added to the FIRST selected task
    // in priority order: Baseboard → Door Casing → Shoe/Quarter Round → Door Hardware
    // The setup time is hidden (not a separate line item) but included in totals
    const TRIM_SETUP_MINUTES = 20;
    let setupTimeApplied = false;

    // 1) Baseboard - Labor: (perimeterFt * 3.0 minutes) / 60 + setup if first
    if (design.installBaseboard && geometry.perimeterFt > 0) {
      const baseboardMinutes = geometry.perimeterFt * 3.0;
      const setupMinutes = !setupTimeApplied ? TRIM_SETUP_MINUTES : 0;
      setupTimeApplied = true;
      const baseboardHours = ((baseboardMinutes + setupMinutes) / 60).toFixed(2);
      laborItems.push({
    ...
    // 2) Door Casing - Labor: (numDoors * 50 minutes) / 60 + setup if first
    if (design.installDoorCasing && geometry.numDoors > 0) {
      const casingMinutes = geometry.numDoors * 50;
      const setupMinutes = !setupTimeApplied ? TRIM_SETUP_MINUTES : 0;
      setupTimeApplied = true;
      const casingHours = ((casingMinutes + setupMinutes) / 60).toFixed(2);
      laborItems.push({
    ...
    // 3) Shoe / Quarter Round - Labor: (perimeterFt * 1.5 minutes) / 60 + setup if first
    if (design.installShoeQuarterRound && geometry.perimeterFt > 0) {
      const shoeMinutes = geometry.perimeterFt * 1.5;
      const setupMinutes = !setupTimeApplied ? TRIM_SETUP_MINUTES : 0;
      setupTimeApplied = true;
      const shoeHours = ((shoeMinutes + setupMinutes) / 60).toFixed(2);
      laborItems.push({
    ...
    // 4) Door Hardware - Labor: (numDoors * 40 minutes) / 60 + setup if first
    if (design.installDoorHardware && geometry.numDoors > 0) {
      const hardwareMinutes = geometry.numDoors * 40;
      const setupMinutes = !setupTimeApplied ? TRIM_SETUP_MINUTES : 0;
      setupTimeApplied = true;
      const hardwareHours = ((hardwareMinutes + setupMinutes) / 60).toFixed(2);
      laborItems.push({
  ...
  useEffect(() => {
    ...
    // Use context items for the check to avoid race condition with local state sync
    const contextLabor = getLaborItems('finishings') || [];
    ...
    if (mergedItems.length > 0 || contextLabor.length > 0) {
  ...
  const carpentryLaborItems = useMemo(() => {
    return localLaborItems.filter(
      (item) => {
        ...
        // Exclude painting items (like "Paint Trim & Baseboards") from carpentry
        if (nameLower.includes('paint')) return false;
        return (
          item.scope === 'carpentry' ||
          nameLower.includes('baseboard') ||
          nameLower.includes('casing') ||
          nameLower.includes('shoe') ||
          nameLower.includes('quarter round') ||
          nameLower.includes('door hardware')
        );
      }
    );
  }, [localLaborItems]);
