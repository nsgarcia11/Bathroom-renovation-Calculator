...
// Formula: rawHours = 12 × (length / 10), clamped 4-24 hrs, rounded to nearest 0.25
...
export default function StructuralLaborSection() {
  ...
  useEffect(() => {
    ...
    // B) Enlarge existing window - 10 hrs @ rate
    ...
    // C) Change doorway opening - 4 hrs @ rate
  ...
  const windowDoorHours = windowDoorItems.reduce(
    (sum, item) => sum + (parseFloat(item.hours) || 0),
    0
  );
            ...
            {isWindowDoorOpen && (
              <>
                {windowDoorItems.length > 0 ? (
                  ...
                  <p className='text-sm text-slate-500 text-center py-4'>
                    No window/door items added yet. Select tasks in Design section.
                  </p>
                )}
              </>
            )}
