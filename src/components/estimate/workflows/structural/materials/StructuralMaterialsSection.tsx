...
export default function StructuralMaterialsSection() {
  ...
  useEffect(() => {
    ...
    const choicesKey = `${design.frameNewWall}-${design.relocateWall}-${design.relocateWallLength}-${design.relocateWallHeight}-${design.removeNonLoadBearingWall}-${design.installBlocking}-${design.frameShowerNiche}-${design.addInsulation}-${design.repairSisterFloorJoists}-${design.levelFloor}-${design.installNewPlywoodSubfloor}-${design.plywoodThickness}-${design.replaceRottenSubfloor}-${design.addNewWindow}-${design.enlargeExistingWindow}-${design.changeDoorwayOpening}-${design.closeOffWindow}`;
    ...
    // F) Add insulation
    // Materials: R-12 batts (qty 2), 6mil poly (qty 1), acoustic sealant (qty 2)
    ...
    // Materials: self leveler (qty 4), primer (qty 1)
    ...
    // Materials: plywood (qty 4, price depends on thickness), flooring screws (qty 1), subfloor glue (qty 4)
    ...
    // Materials: plywood (qty 2), blocking studs (qty 4)
    ...
    // A) Add new window
    // Materials: header (qty 2), 2x4 framing (qty 6), flashing tape (qty 1)
    if (design.addNewWindow) {
      newMaterialItems.push(
        ...
        {
          id: 'add-window-framing',
          name: '2×4 framing lumber',
          quantity: '6',
          price: '3.38',
          unit: 'each',
          category: 'construction',
    ...
    // B) Enlarge existing window
    // Materials: header (qty 2), 2x4 (qty 4), temp support (qty 4)
    if (design.enlargeExistingWindow) {
      newMaterialItems.push(
        ...
        {
          id: 'enlarge-window-support',
          name: 'Temp support lumber',
          quantity: '4',
          price: '5.00',
          unit: 'each',
          category: 'construction',
    ...
    // D) Close off window
    // Materials: 2x4 (qty 5), sheathing (qty 1), insulation (qty 1)
  ...
  const windowDoorTotal = windowDoorItems.reduce(
    (sum, item) =>
      sum + (parseFloat(item.quantity) || 0) * (parseFloat(item.price) || 0),
    0
  );
            ...
            {isWindowDoorOpen && (
              <>
                {windowDoorItems.length > 0 ? (
                  ...
                  <p className='text-sm text-slate-500 text-center py-4'>
                    No window/door materials added yet. Select tasks in Design section.
                  </p>
                )}
              </>
            )}
