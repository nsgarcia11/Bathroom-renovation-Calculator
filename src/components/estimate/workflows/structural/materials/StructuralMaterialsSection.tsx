...
export default function StructuralMaterialsSection() {
  ...
  // Handlers
  const handleAddWallWorkMaterialItem = useCallback(() => {
  ...
  const handleAddStructuralSupportMaterialItem = useCallback(() => {
    userActionRef.current = true;
    const newItem: MaterialItem = {
  ...
  const handleAddSubfloorWorkMaterialItem = useCallback(() => {
    userActionRef.current = true;
    const newItem: MaterialItem = {
  ...
  const handleAddInsulationMaterialItem = useCallback(() => {
    userActionRef.current = true;
    const newItem: MaterialItem = {
  ...
  const wallWorkMaterials = useMemo(() => {
    return localMaterialItems.filter(
      (item) =>
        ...
        item.name.toLowerCase().includes('lumber')
    );
  }, [localMaterialItems]);

  const structuralSupportMaterials = useMemo(() => {
    return localMaterialItems.filter(
      (item) =>
  ...
  const subfloorWorkMaterials = useMemo(() => {
    return localMaterialItems.filter(
      (item) =>
        ...
        item.name.toLowerCase().includes('osb')
    );
  }, [localMaterialItems]);
  ...
  const insulationMaterials = useMemo(() => {
    return localMaterialItems.filter(
      (item) =>
        ...
        item.name.toLowerCase().includes('fiberglass')
    );
  }, [localMaterialItems]);

  // Calculate totals for each category
  const wallWorkTotal = wallWorkMaterials.reduce(
    (sum, item) =>
      sum + (parseFloat(item.quantity) || 0) * (parseFloat(item.price) || 0),
    0
  );

  const structuralSupportTotal = structuralSupportMaterials.reduce(
    (sum, item) =>
      sum + (parseFloat(item.quantity) || 0) * (parseFloat(item.price) || 0),
    0
  );

  const subfloorWorkTotal = subfloorWorkMaterials.reduce(
    (sum, item) =>
      sum + (parseFloat(item.quantity) || 0) * (parseFloat(item.price) || 0),
    0
  );

  const insulationTotal = insulationMaterials.reduce(
    (sum, item) =>
      sum + (parseFloat(item.quantity) || 0) * (parseFloat(item.price) || 0),
    0
  );
      ...
      <div className='space-y-4'>
        ...
        <Card>
          <CardContent className='p-3 space-y-3'>
            ...
            {isWallWorkOpen && (
              <>
                {wallWorkMaterials.length > 0 ? (
                  <div className='space-y-2'>
                    {wallWorkMaterials.map((item) => (
                      <div
                        key={item.id}
                  ...
                  <p className='text-sm text-slate-500 text-center py-4'>
                    No wall work materials added yet.
                  </p>
                )}
                <div className='flex justify-center'>
                  <Button
                    onClick={handleAddWallWorkMaterialItem}
                    variant='default'
        ...
        {/* Structural Support Materials Section */}
        <Card>
          <CardContent className='p-3 space-y-3'>
            <div
              className='flex justify-between items-center cursor-pointer'
            ...
            >
              <div className='flex items-center gap-2 justify-between w-full'>
                ...
                <p className='font-bold text-blue-600 text-sm'>
                  ${structuralSupportTotal.toFixed(2)}
                </p>
              </div>
            </div>
            {isStructuralSupportOpen && (
              <>
                {structuralSupportMaterials.length > 0 ? (
                  <div className='space-y-2'>
                    {structuralSupportMaterials.map((item) => (
                      <div
                        key={item.id}
                        className={`p-3 bg-white rounded-lg border ${
                          item.isAutoGenerated
                  ...
                  <p className='text-sm text-slate-500 text-center py-4'>
                    No structural support materials added yet.
                  </p>
                )}
                <div className='flex justify-center'>
                  <Button
                    onClick={handleAddStructuralSupportMaterialItem}
                    variant='default'
                    className='w-auto mt-2 flex items-center justify-center gap-2 py-2.5 px-4 text-white font-semibold border-blue-200'
                  >
                    <Plus size={16} />
        ...
        {/* Subfloor Work Materials Section */}
        <Card>
          <CardContent className='p-3 space-y-3'>
            <div
              className='flex justify-between items-center cursor-pointer'
            ...
            >
              <div className='flex items-center gap-2 justify-between w-full'>
                ...
                <p className='font-bold text-blue-600 text-sm'>
                  ${subfloorWorkTotal.toFixed(2)}
                </p>
              </div>
            </div>
            {isSubfloorWorkOpen && (
              <>
                {subfloorWorkMaterials.length > 0 ? (
                  <div className='space-y-2'>
                    {subfloorWorkMaterials.map((item) => (
                      <div
                        key={item.id}
                        className={`p-3 bg-white rounded-lg border ${
                          item.isAutoGenerated
                  ...
                  <p className='text-sm text-slate-500 text-center py-4'>
                    No subfloor work materials added yet.
                  </p>
                )}
                <div className='flex justify-center'>
                  <Button
                    onClick={handleAddSubfloorWorkMaterialItem}
                    variant='default'
                    className='w-auto mt-2 flex items-center justify-center gap-2 py-2.5 px-4 text-white font-semibold border-blue-200'
                  >
                    <Plus size={16} />
            ...
            {isInsulationOpen && (
              <>
                {insulationMaterials.length > 0 ? (
                  ...
                  <p className='text-sm text-slate-500 text-center py-4'>
                    No insulation materials added yet.
                  </p>
                )}
                <div className='flex justify-center'>
                  <Button
                    onClick={handleAddInsulationMaterialItem}
                    variant='default'
                    className='w-auto mt-2 flex items-center justify-center gap-2 py-2.5 px-4 text-white font-semibold border-blue-200'
                  >
                    <Plus size={16} />
