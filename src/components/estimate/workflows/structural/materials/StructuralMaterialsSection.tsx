...
export default function StructuralMaterialsSection() {
  ...
  // Calculate floor area in sq ft from floors tab measurements
  const floorAreaSqFt = useMemo(() => {
    ...
    // Round to nearest whole number, minimum 1 if no measurements
    return totalArea > 0 ? Math.round(totalArea) : 1;
  }, [floorsDesignData]);
  ...
  useEffect(() => {
    ...
    // Materials: plywood (qty = floor sq ft), flooring screws (qty 1), subfloor glue (qty 4)
      ...
      newMaterialItems.push(
        {
          ...
          quantity: floorAreaSqFt.toString(),
    ...
    // Materials: plywood (qty = floor sq ft), blocking studs (qty 4)
  ...
  }, [design, studCount, floorAreaSqFt, isReloading, setMaterialItems, contextMaterialItems]);
