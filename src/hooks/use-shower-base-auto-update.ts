...
interface UseShowerBaseAutoUpdateProps {
...
export function useShowerBaseAutoUpdate({
  measurements,
  design,
  construction,
  currentLaborItems,
  ...
  onUpdateLabor,
  onUpdateMaterials,
}: UseShowerBaseAutoUpdateProps) {
  // Generate auto labor and materials
  const autoLaborItems = ShowerBaseCalculator.generateAutoLabor(
    measurements,
    design,
    construction
  );
  const autoMaterialItems = ShowerBaseCalculator.generateAutoMaterials(
    measurements,
    design,
    construction
  );
  ...
  useEffect(() => {
    ...
    const filteredLaborItems = reconciledLaborItems.filter((item) => {
      // Keep all custom items (not starting with 'sb-')
      if (!item.id.startsWith('sb-')) {
        return true;
      }
